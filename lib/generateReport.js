...
const { getSimpleCypressConfig, convertJsonScreenShotsToBase64 } = require('./utils');
...
async function mergeAndCreate(jsonDir, mochawesomeOptions) {
  ...
  if (mochawesomeOptions.embeddedScreenshots == true) {
    ...
    await convertJsonScreenShotsToBase64(jsonDir);
    console.log(`Embedding screenshots successfully`);
  }
...
async function generateReport(config) {
  ...
  const actions = [mergeAndCreate(jsonDir, reporterOptions)];
