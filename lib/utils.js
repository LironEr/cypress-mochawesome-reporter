...
async function convertJsonScreenshotsToBase64(jsonsDir,screenshotsDir) {
  ...
  function convertContextImgToBase64(contextObj){
    if (contextObj.value) {
  ...
  function genereatebase64Context(context,screenshotsDir) {
    if(context == null)
      return null;

    const contextObj = JSON.parse(context);

    if(contextObj.length && contextObj.length >= 1)
    {
      for (let i = 0; i < contextObj.length; i++) {
        const element = contextObj[i];
        convertContextImgToBase64(element);
      }
    }
    else 
      convertContextImgToBase64(contextObj);
    ...
    for (let i = 0; i < json.results.length; i++) {
      ...
      for (let z = 0; z < result.suites.length; z++) {
        ...
        for (let y = 0; y < suite.tests.length; y++) {
          ...
          test.context = genereatebase64Context(test.context,screenshotsDir);
        }
      }
    }
