  ...
  return screenshots.map((screenshot) => ({
    ...
    value:
      mochawesomeOptions.embeddedScreenshots === true
        ...
        : encodeScreenshotPath(`screenshots${screenshot.value}`),
  }));
}
...
function convertImageToBase64(screenshotsDir, imagePath) {
  ...
  if (!fse.existsSync(imgPath)) {
    log(`Expected screenshot not found at: ${imagePath}. Image will be ommitted from mochawesome report.`);
    return '';
  }

  return 'data:image/png;base64, ' + fse.readFileSync(imgPath, { encoding: 'base64' });
}

function encodeScreenshotPath(p) {
