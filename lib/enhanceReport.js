...
function attachMediaToTestContext(context, mochawesomeOptions, screenshotsDir, baseFolder) {
  ...
  const isScreenshotContext = (c) => typeof c === 'object' && c.title === 'cypress-mochawesome-reporter-screenshots';
  const screenshotsItems = parsedContext.filter(isScreenshotContext);

  debugLog(`screenshots entries found: ${screenshotsItems.length}`);

  if (screenshotsItems.length > 0) {
    const screenshots = screenshotsItems[0].value || [];
    // remove all raw screenshot entries
    parsedContext = parsedContext.filter((c) => !isScreenshotContext(c));
...
function createScreenshotsContextList(screenshots, titleSuffix, mochawesomeOptions, screenshotsDir) {
  // If disabled by config, do not attach screenshots to the HTML context
  if (mochawesomeOptions.addScreenshotToHTML === false) {
    return [];
  }
